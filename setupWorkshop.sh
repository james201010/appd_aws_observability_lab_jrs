...
if [ ! -d /opt/appdynamics ]; then
  ...
  git clone https://github.com/james201010/appd_aws_observability_lab_jrs.git appd_lab_repo
  curl --silent -L https://raw.githubusercontent.com/james201010/appd_aws_observability_lab_jrs/main/teardownWorkshop.sh -o /home/ec2-user/environment/teardownWorkshop.sh
  ...
  cp /opt/appdynamics/appd_lab_repo/scripts/getIpsForCloudDbSecurityGroup.sh /opt/appdynamics/workshopuser/
...
/opt/appdynamics/workshopuser/resizeAl2EbsVolume.sh


# check to see if user_id file exists and if so read in the user_id
if [ -f "/opt/appdynamics/workshopuser/state/appd_workshop_user.txt" ]; then

  appd_workshop_user=$(cat /opt/appdynamics/workshopuser/state/appd_workshop_user.txt)
  ...
  echo "$appd_workshop_user" > /opt/appdynamics/workshopuser/state/appd_workshop_user.txt


  # write the C9 user to a file     example:  james.schneider
  echo "$C9_USER" > /opt/appdynamics/workshopuser/state/appd_env_user.txt

  # write the Hostname to a file   example:  ip-172-31-14-237.us-west-1.compute.internal
  echo "$HOSTNAME" > /opt/appdynamics/workshopuser/state/appd_env_host.txt

fi	

# !!!!!!! BEGIN BIG IF BLOCK !!!!!!!
if [ -f "/opt/appdynamics/workshopuser/state/appd_workshop_setup.txt" ]; then

  appd_wrkshp_last_setupstep_done=$(cat /opt/appdynamics/workshopuser/state/appd_workshop_setup.txt)
...
echo "$appd_wrkshp_last_setupstep_done" > /opt/appdynamics/workshopuser/state/appd_workshop_setup.txt
